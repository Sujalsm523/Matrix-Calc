...
import MatrixModifiers, {
  createInitialMatrix,
  matrixCanExpandCols,
  matrixCanExpandRows,
} from "../../lib/MatrixModifiers";
...
export default class MatrixGridController extends React.Component {
  ...
  setMatrixValue(ev, row, col) {
    ...
    this.setMatrixState(
      MatrixModifiers.setElementValue(this.state.matrix, row, col, value)
    );
  }

  // Converts a given matrix value into a Number
  parseMatrixValue = (row, col) =>
    this.setMatrixState(
      MatrixModifiers.setElementValue(
        this.state.matrix,
        row,
        col,
        Number(this.state.matrix[row][col]) || 0
      )
    );

  expandMatrixCols = () =>
    this.setMatrixState(MatrixModifiers.addColumn(this.state.matrix));

  expandMatrixRows = () =>
    this.setMatrixState(MatrixModifiers.addRow(this.state.matrix));

  expandMatrixRowsAndCols = () =>
    this.setMatrixState(MatrixModifiers.addRowAndColumn(this.state.matrix));

  removeRow = (index) =>
    this.setMatrixState(MatrixModifiers.removeRow(this.state.matrix, index));

  removeColumn = (index) =>
    this.setMatrixState(MatrixModifiers.removeColumn(this.state.matrix, index));
        ...
        <table style={{ height: "100%" }}>
          <tbody>
            <tr>
              <td>{/* Empty cell */}</td>
              {!this.readonly &&
                this.state.matrix[0].map((c, index) => (
                  <RemoveButton
                    key={index}
            ...
            {this.state.matrix.map((row, rowIndex) => (
              <tr key={rowIndex} style={{ height: "100%" }}>
                ...
                {!this.readonly && (
                  <RemoveButton onClick={() => this.removeRow(rowIndex)} />
                )}
                ...
                {!this.readonly &&
                rowIndex === 0 &&
                matrixCanExpandCols(this.state.matrix) ? (
                  <td
                    rowSpan={this.state.matrix.length}
                    style={{ paddingBottom: "8px" }}
                  >
                    <button
                      className="button light w-full divide-violet-700"
                      style={{ height: "100%" }}
            ...
            <tr>
              ...
              {!this.readonly && matrixCanExpandRows(this.state.matrix) ? (
                <td
                  colSpan={this.state.matrix[0].length}
                  style={{ paddingRight: "8px" }}
                >
                  <button
                    className="button light w-full"
                    onClick={this.expandMatrixRows.bind(this)}
                  >
                    +
                  </button>
                </td>
              ...
              {!this.readonly &&
              matrixCanExpandRows(this.state.matrix) &&
              matrixCanExpandCols(this.state.matrix) ? (
                <td>
                  <button
                    className="button light w-full"
  ...
  return (
    <td style={{ paddingRight: 8 }}>
      <div className={styles.removeButtonCell}>
        <span onClick={onClick} className={styles.removeButton}>
          -
        </span>
      </div>
    </td>
