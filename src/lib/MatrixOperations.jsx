...
export default class MatrixOperations {
    ...
    try {
      switch (operation) {
        ...
        case OP_SQUARE:
          ...
          resultMatrix = MatrixOperations.multiply(
            MatrixOperations.multiply(matrixA, matrixA),
            matrixA
          );
          break;
        case OP_DETERMINANT:
      ...
      return [
        null,
        error ||
          "Error while performing an operation with the provided matrices",
      ];
    }
  ...
  static multiplyByConst(matrix, number) {
    return matrix.map((row) => row.map((el) => el * number));
  }
  ...
  static findDeterminant(m) {
    ...
    firstRow.forEach((val, col) => {
      result +=
        val * Math.pow(-1, col + 1 + 1) * MatrixOperations.getMinor(m, 0, col);
    });
  ...
  static getMinor(m, rowIndex, colIndex) {
    ...
    result = result.map((matrixRow) =>
      matrixRow.filter((el, col) => col !== colIndex)
    );
...
export const canMultiplyMatrices = (a, b) => getColCount(a) === getRowCount(b);
